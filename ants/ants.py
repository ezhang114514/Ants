...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def find_nearest_bee(place,lower_bound,upper_bound,number):
            if(place.is_hive or not(place.bees) or not((number>=lower_bound)and(number<=upper_bound))):
                if(place.entrance):
                    return find_nearest_bee(place.entrance,lower_bound,upper_bound,number+1)
                else:
                    return None
            else:
                return random_bee(place.bees)
        return find_nearest_bee(self.place,self.lower_bound,self.upper_bound,0)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bees in self.place.bees[:]:
            bees.reduce_health(amount+self.damage*(self.health-amount<=0))
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.turns_to_chew==0:
            if self.place.bees:
                a_bee=random_bee(self.place.bees)
                Ant.reduce_health(a_bee,a_bee.health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if(self.is_container==True and self.ant_contained==None and other.is_container!=True):
            return True
        else:
            return False
        # END Problem 8a
    ...
    def action(self, gamestate):
        ...
        if(self.ant_contained!=None):
            self.place=self.ant_contained.place
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        def double_damage(self,location):
            if(location.exit!=None):
                if(location.exit.ant!=None and not(location.exit.ant in self.doubled)):
                    location.exit.ant.damage*=2
                    self.doubled.append(location.exit.ant)
                if(location.exit.ant!=None and location.exit.ant.is_container and location.exit.ant.ant_contained!=None and not(location.exit.ant.ant_contained in self.doubled)):
                    location.exit.ant.ant_contained.damage*=2
                    self.doubled.append(location.exit.ant.ant_contained)
                double_damage(self,location.exit)
        double_damage(self,self.place)
        # END Problem 12
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        dicti={}
        def fill_the_dicti(place,distance):
            if(place.bees):
                for bee in place.bees:
                    dicti[bee]=distance
            if(place.ant):
                if(place.ant is not self):
                    dicti[place.ant]=distance
                if(place.ant.is_container and place.ant.ant_contained):
                    dicti[place.ant.anr_contained]=distance
            if(place.entrance!=None):
                fill_the_dicti(place.entrance,distance+1)
        fill_the_dicti(self.place,0)
        return dicti
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        new_damage=self.damage-0.25*distance
        self.damage-=0.0625
        if(new_damage<=0):
            return 0
        return new_damage
